...
int write_num(int ind, char buffer[], int flags, int width,
	int precision, int length, char padd, char extra_ch)
{
	int i, padd_start = 1;
	...
	return (0);
	if (precision == 0 && BUFF_SIZE - 2 && buffer[ind] == '0')
	buffer[ind] = padd = ' ';
 ...
 * @is_negative: indictate if the number is negative
